...
- 접속 정보·계정은 [`GPU_서버_사용가이드.md`](GPU_서버_사용가이드.md) 에 있습니다
...
1. **`Beauty_split_B.pkl` 은 학습 데이터가 너무 적어 아직 콜드스타트 실험에 못 씁니다.**
   train이 전체의 0.04~2.62%뿐이고 W3는 아이템 12,101개 중 12,032개가 `unseen` 입니다.
   (🔄 정정: 처음엔 "분위수 계산 버그"로 알렸는데 아닙니다. 전체 기간을 균등한 달력 기간으로
   5등분하는 논문 방식이 의도대로 구현된 것이고, 리뷰가 2013년에 몰려 있어 생긴 결과입니다.
   프로토콜을 그대로 쓸지가 논의 대상입니다.)
   그래프 SID의 콜드스타트 효과를 보시려면 이 결정이 먼저 나야 합니다.
