...
팀 노션 페이지 **8/20** 에 지금까지 결과가 정리돼 있는데,
**전수 검증 결과와 트랙 C 결과가 아직 반영되지 않았습니다.**
트랙 C 가 끝나면 한 번에 갱신하기로 팀과 합의돼 있습니다.

- 페이지: https://app.notion.com/p/3c294be330d68044a9f4ec337348a832
- 반영해야 할 것
  1. `clip_L4_fullval` 결과 (R@5 0.0441 / R@10 0.0654) — 표본 검증과 성능 동일
  2. 4절 "7,000스텝 주기 진동" 서술 정정 → 검증 표본의 착시였음
  3. 최종 레시피에 **전수 검증(LIMIT_VAL=1.0)** 추가
  4. 트랙 C 결과 (다양성 지표, 파레토 곡선)

자세한 정정 내용은 `docs/TIGER_BASELINE_REPORT.md` 3절 참고.
