...
**① Split B의 시간 분할이 설계와 다름.**
"80/85/90/95% 분위수 컷 → 항상 약 8:2" 설계인데 실측은:
...
train이 전체의 0.04~2.6%뿐이고 W5의 cold_tiers가 12,101개 중 12,032개 `unseen`.
분위수를 리뷰 건수가 아니라 **고유 타임스탬프(날짜) 기준으로 계산**했거나 부등호가 반대로
된 것으로 추정. **이대로는 콜드스타트 실험(공식 결과 B분할)에 사용 불가.**
